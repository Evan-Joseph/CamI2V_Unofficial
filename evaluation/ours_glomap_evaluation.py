...
def metric(c2w_1: Tensor, c2w_2: Tensor) -> tuple[float, float, float]:  # N, 3, 4
    # 添加尺寸检查
    if c2w_1.shape[0] != c2w_2.shape[0]:
        print(f"Warning: Frame count mismatch - GT: {c2w_1.shape[0]}, Reconstructed: {c2w_2.shape[0]}")
        # 取较小的帧数进行评估
        min_frames = min(c2w_1.shape[0], c2w_2.shape[0])
        c2w_1 = c2w_1[:min_frames]
        c2w_2 = c2w_2[:min_frames]
